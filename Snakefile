...
rule all:
	input:
		...
		expand(f"{OUTPUT_DIR}/{{sample}}_assembly/contigs.fasta", sample=SAMPLES),
		...
		expand(f"{OUTPUT_DIR}/{{sample}}_gtdbtk_temp", sample=SAMPLES) if config["sub_analyses"].get("mags", False) else [],
